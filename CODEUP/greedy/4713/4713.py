'''
이미지 속의 내용을 텍스트로 정확하게 옮겨 드립니다. 필요하신 곳에 편하게 복사해서 사용하세요!
...
첫째 줄에는 꽃들의 총 개수 $N$ ($1 \le N \le 100,000$)이 주어진다.

다음 $N$개의 줄에는 각 꽃이 피는 날짜와 지는 날짜가 주어진다. 
하나의 날짜는 월과 일을 나타내는 두 숫자로 표현된다. 
예를 들어, **3 8 7 31**은 꽃이 **3월 8일**에 피어서 **7월 31일**에 진다는 것을 나타낸다.
...
for _ in range(n):
    ...
    bloom.append((sm +dec(sd), fm + dec(fd)))

bloom.sort(key=lambda x: x[0])
j = 0
